...
export async function showExamHistory({
  ...
  loadExamAttempts(user.uid),
  loadProgressiveAttempts(user.uid)
]);
      ...
      <section class="exam-history-content">
        <div class="exam-history-section-heading">
          ...
          <span class="exam-history-count">
            ${progressiveAttempts.length}
            ${
              progressiveAttempts.length === 1
                ? "risultato"
                : "risultati"
            }
          </span>
        ...
        ${
          progressiveAttempts.length === 0
            ? createProgressiveEmptyTemplate()
            : createProgressiveAttemptsTemplate(
                progressiveAttempts
              )
        }
      </section>
...
async function loadProgressiveAttempts(uid) {
  const progressiveReference = collection(
    db,
    "users",
    uid,
    "progressiveProgress"
...
function getHistoryTimestamp(rawDate) {
  if (!rawDate) {
    return 0;
  }

  if (
    typeof rawDate.toMillis ===
    "function"
  ) {
  ...
  if (rawDate.seconds) {
    return Number(rawDate.seconds) * 1000;
  }
...
function renderExamHistory({
  ...
  document
    .querySelectorAll(
      "[data-progressive-attempt-index]"
    )
    .forEach((button) => {
      button.addEventListener(
        "click",
        () => {
          ...
          if (!progressiveAttempt) {
            return;
          }

          renderProgressiveDetails({
            container,
            attempt:
              progressiveAttempt,
...
function createProgressiveAttemptsTemplate(
  progressiveAttempts
) {
  return `
    <div class="exam-history-list">
      ${progressiveAttempts
        .map((attempt, index) => {
          const totalQuestions =
            getProgressiveTotalQuestions(
              attempt
            );

          const correctAnswers =
            getProgressiveCorrectAnswers(
              attempt
            );

          const wrongAnswers =
            getProgressiveWrongAnswers(
              attempt
            );
          ...
          const lessonCount =
            attempt.selectedLessonIds
              ?.length ||
            attempt.selectedLessonTitles
              ?.length ||
            0;

          const date =
            formatProgressiveDate(
              attempt.createdAt
            );
          ...
          return `
            ...
            >
              <div
                class="exam-history-item-status"
              >
                <div
                  class="
                    exam-history-result-icon
                    exam-history-result-icon-passed
                  "
                >
                  📚
                ...
                <div>
                  <span
                    class="exam-history-attempt-label"
                  >
                    Quiz Progressivo
                  </span>

                  <strong
                    class="exam-history-passed-text"
                  >
                    ${escapeHtml(
                      attempt.topicTitle
                    )}
              ...
              <div
                class="exam-history-item-metrics"
              >
                <div>
                  <span>Punteggio</span>
              ...
              <button
                type="button"
                class="exam-history-review-button"
                data-progressive-attempt-index="${index}"
              >
                Vedi dettagli
...
function createProgressiveEmptyTemplate() {
  return `
    <section class="exam-history-empty">
      <div
        class="exam-history-empty-icon"
      >
        📚
      </div>

      <h3>
        Nessun Quiz Progressivo completato
      </h3>

      <p>
        Completa un Quiz Progressivo per
        vedere qui punteggio, lezioni
        selezionate e XP ottenuti.
      </p>
    </section>
  `;
}

function renderProgressiveDetails({
  container,
  attempt,
  onBackToHistory
}) {
  ...
  container.innerHTML = `
    <main class="exam-review-page">
      ...
      >
        <div
          class="exam-review-summary-result"
        >
          <div
            class="
              exam-history-result-icon
        ...
        <div
          class="exam-review-summary-grid"
        >
          <div>
      ...
      <section
        class="exam-review-content"
      >
        <div
          class="exam-history-section-heading"
        >
          <div>
            <p
              class="exam-history-eyebrow"
            >
              Lezioni selezionate
            </p>
        ...
        ${
          lessons.length > 0
            ...
            : `
                <section
                  class="exam-history-empty"
                >
                  <div
                    class="
                      exam-history-empty-icon
                    "
                  >
                    📖
                  </div>
                  ...
                  <p>
                    ...
                    salvato senza i nomi
                    delle lezioni selezionate.
                  </p>
                </section>
              `
        }
...
function getProgressiveTotalQuestions(
  attempt
) {
...
function getProgressiveCorrectAnswers(
  attempt
) {
  ...
  if (Number.isFinite(savedCorrect)) {
    return savedCorrect;
  }
...
function getProgressiveWrongAnswers(
  attempt
) {
  const savedWrong = Number(
    attempt.wrongAnswers
  );

  if (Number.isFinite(savedWrong)) {
    return savedWrong;
  }
...
function formatProgressiveDate(
  rawDate
) {
  if (!rawDate) {
...
function createEmptyTemplate() {
  return `
    <section class="exam-history-empty">
      ...
      <p>
  Completa una Simulazione Esame per vedere qui
