...
import {
  selectBalancedMinisterialQuestions
} from "../utils/selectBalancedMinisterialQuestions.js";
...
export async function openMinisterialFlow(
  app,
  user,
  actions
) {
  ...
  try {
    ministerialQuestions =
      await loadPublishedMinisterialQuestions();
  } catch (error) {
    console.error(
      "Published ministerial questions loading error:",
      error
    );
...
const EXAM_STORAGE_KEY =
  ...
  const MINISTERIAL_CLOUD_DRAFT_ID =
  createCloudDraftId(
    EXAM_STORAGE_KEY
  );
...
async function loadPublishedMinisterialQuestions() {
...
async function showMinisterialStartPage(
  app,
  user,
  actions
) {
...
if (savedDraft) {
  try {
    ...
    console.warn(
      "Ministerial local draft restore error:",
      error
    );
  }
}
              ...
              <div class="exam-ready-message">
                ...
                <div>
                  ...
                  <p>
                    Sono disponibili
                    ${availableQuestions}
                    domande pubblicate in Firestore.
                  </p>
                </div>
              </div>
            ...
            : `
              <div class="exam-not-ready-message">
                ...
                <div>
                  ...
                  <p>
                    Per avviare una simulazione servono almeno
                    ${config.questionCount}
